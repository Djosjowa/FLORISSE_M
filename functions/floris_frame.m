function [ wt_order,sortvector,site,wt_locations_wf,shiftvec ] = floris_frame( site,turb,wt_locations_if )
...
shiftvec = [min(wt_locations_wf(:,1)); min(wt_locations_wf(:,2))];
wt_locations_wf(:,1) = wt_locations_wf(:,1)-shiftvec(1); % shift horizontally (sideways)
wt_locations_wf(:,2) = wt_locations_wf(:,2)-shiftvec(2); % shift vertically (up-down)
